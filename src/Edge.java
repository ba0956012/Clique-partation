...
public class Edge implements Comparable<Edge> {
    ...
    public boolean discriminantCommonNeighbor(Vertex v){
		
    	boolean one = false;
    	boolean two = false;
    	for(int i=0; i<v.getNeighborCount();i++){
    		if(v.getNeighbor(i).one.getLabel()==this.one.getLabel()||v.getNeighbor(i).two.getLabel()==this.one.getLabel()){
    			one = true;
    		}
    		if(v.getNeighbor(i).one.getLabel()==this.two.getLabel()||v.getNeighbor(i).two.getLabel()==this.two.getLabel()){
    			two = true;
    		}
    ...
    public List<Vertex> getCommonNeighbors(){
    	List<Vertex> V = new ArrayList();
    	
    	for(int i=0; i<this.one.getNeighborCount();i++){
    		if(this.discriminantCommonNeighbor(this.one.getNeighbor(i).one)==true
    				&&this.one.getNeighbor(i).one != this.one
    				&&this.one.getNeighbor(i).one!=this.two){
    			V.add(this.one.getNeighbor(i).one);
    		}
    		if(this.discriminantCommonNeighbor(this.one.getNeighbor(i).two)==true
    				&&this.one.getNeighbor(i).two != this.one
    				&&this.one.getNeighbor(i).two!=this.two){
    			V.add(this.one.getNeighbor(i).two);
    		}
    	}
     ...
     */
    public int hashCode(){
    	//System.out.println("hashCode: "+(one.getLabel() + two.getLabel()).hashCode());
